...
### Prompt-Semantic Gap in Open-Vocabulary Detectors

Running Grounding DINO and OWL-ViT on synthetic robotic manipulation scenes reveals a consistent failure mode: **open-vocabulary detectors trained on natural images underperform on flat synthetic renders because synthetic scenes lack the texture, lighting, and material cues these models rely on.**

Key observations:

- **Grounding DINO** achieves IoU 0.95 on prompts with direct colour-name matches (`"red box"`) but near-zero on shape/material prompts (`"blue can"`, `"green bowl"`). The model is prompt-semantic: it pattern-matches language tokens against co-occurring visual features from COCO/OpenImages training data. Flat-shaded rectangles provide no material or shape context.
- **OWL-ViT** (CLIP-based) achieves 0% recall on all synthetic frames. CLIP's image encoder is heavily texture-driven — the same reason CLIP famously misclassifies images with shuffled patches. Un-raytraced renders break its visual prior entirely.
- **The 3D error jump (2 mm → 257 mm) is prompt-driven, not geometric.** Pinhole deprojection is identical across evaluations. When a detector mislocalises a box onto the background, depth sampling hits the table surface (~1.5 m) instead of the object (~0.78 m), inflating 3D error. On matched prompts, error is in the 10–30 mm range. Day 6 fix: replaced box-centre depth sampling with label-mask centroid + median depth over mask pixels, which eliminates background contamination entirely.

### Implication for Sim-to-Real Transfer

This is a known open problem: models trained on real-world image distributions do not transfer zero-shot to synthetic renders without either (a) domain randomisation at render time, (b) fine-tuning on synthetic data, or (c) photorealistic rendering (e.g. Isaac Sim path tracing). OmniGrasp Week 3 target: re-evaluate on Isaac Sim photorealistic scenes to quantify the gap closure from raytraced rendering.
